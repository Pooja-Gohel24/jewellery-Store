...
from app.schemas.user import RegisterRequest, LoginRequest, OTPRequest, TokenResponse, UserResponse, MessageResponse
...
def resend_otp(payload: LoginRequest, db: Session = Depends(get_db)):
    user = get_user_by_email(db, payload.email)
    if not user or not verify_password(payload.password, user.hashed_password):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
